...
if (!is_object($db))
{
	die('<strong>MySQL 错误</strong>: $db 不是 vB Database 类的一个实例。本脚本需要 vB Database 类中的 escape_string() 方法。');
}
...
CREATE TABLE " . TABLE_PREFIX . "post (
	...
	titleindex VARCHAR(250) NOT NULL DEFAULT '',
	pagetextindex MEDIUMTEXT,
	PRIMARY KEY (postid),
	KEY userid (userid),
	KEY threadid (threadid, userid),
	FULLTEXT KEY title (titleindex, pagetextindex)
) $enginetype=MyISAM
";
// vB 中文修改结束
...
CREATE TABLE " . TABLE_PREFIX . "thread (
	...
	KEY dateline (dateline),
	FULLTEXT KEY titleindex (titleindex)
) $enginetype=MyISAM
";
// vB 中文修改结束
...
// vB 中文修改开始：中文搜索修改 title 字段。原为 CHAR(50)。增加 Word 分数字段及日期字段
