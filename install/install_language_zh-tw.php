...
$vbphrase['proceed'] = '繼續';
$vbphrase['reset'] = '重置';
$vbphrase['vbulletin_copyright'] = 'vBulletin v' . VERSION . ', 版權所有 &copy;2000 - 2007, Jelsoft Enterprises Ltd.';
...
$install_phrases['welcome'] = '<p style="font-size:10pt"><b>歡迎使用 vBulletin 3.6 中文版</b></p>
	<p>您即將開始裝載本程式。</p>
...
$install_phrases['cookiepath'] = '<b>Cookie 儲存路徑</b> <dfn>Cookie 儲存的路徑。如果您在同一個域名下執行了多個論壇，便需要將它設定為每個論壇所在的目録名。否則，填寫 / 便可以了。<br /><br />Cookie 路徑建議的有效值已在旁邊的下拉框中列出。如果您認為還有更加合適的不同的設定，選中核取框并在下面的匯入框中匯入合適的值。<br /><br />請注意路徑<b>總是</b>應當以正斜杠結尾，例如 \'/forums/\'、\'/vbulletin/\' 等。<br /><br /><span class="col-c">匯入錯誤的路徑，會導致您無法登入論壇。</span></dfn>';
$install_phrases['cookiedomain'] = '<b>Cookie 范圍名</b> <dfn>Cookie 所影響的域名。修改此選項預設值最常見的原因是，您的論壇有兩個不同的網址，例如 example.com 和 forums.example.com。要使用户在以兩個不同的域名存取論壇時，都能保持登入狀態，您需要將此選項設定為 <b>.example.com</b> (注意域名需要以<b>點</b>開頭)。<br /><br />Cookie 域名建議的有效值已在旁邊的下拉框中列出。如果您認為還有更加合適的不同的設定，選中核取框并在下面的匯入框中匯入合適的值。<br /><br /><span class="col-c">若您没有把握，最好在這裡什麽也不填寫，因為錯誤的設定會導致您無法登入論壇。</span></dfn>';
