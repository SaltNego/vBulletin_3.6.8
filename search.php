...
// vB 中文修改开始：中文搜索 全文搜索
					if ($vbulletin->options['searchposttable'])
					{					
							$words['NOT']["$word"] = 'NOT';
...
// vB 中文修改开始：中文搜索
							...
							$getwords = $db->query_read_slave("
								SELECT wordid, title FROM " . TABLE_PREFIX . "word
									WHERE title LIKE('" . sanitize_word_for_sql($queryword) . "')
							");
							}
							if ($vbulletin->options['searchposttable'] OR $db->num_rows($getwords))
							{
								if (!$vbulletin->options['searchposttable'])
							{
								...
								");
// vB 中文修改开始：中文搜索，索引词汇分数更新
									$db->query_write("UPDATE " . TABLE_PREFIX . "word SET score = score+1 WHERE wordid IN(" . implode(',', $wordids) . ")"); 
// vB 中文修改结束
								}
								else
								{
									$postmatches = $db->query_read("
									    ...
									    WHERE title LIKE '%" . sanitize_word_for_sql($word) . "%'" . iif(!$vbulletin->GPC['titleonly'], " OR pagetext LIKE '%" . sanitize_word_for_sql($word) . "%'")
									);
								}
// vB 中文修改结束
...
// vB 中文修改开始：中文搜索
									if ($vbulletin->GPC['titleonly'] AND !$vbulletin->options['searchposttable'])
// vB 中文修改结束
									{
...
// vB 中文修改开始: 全文搜索
						...
						{
							$thread_query_logic[] = "MATCH(thread.titleindex) AGAINST ('$wordlist' IN BOOLEAN MODE)";
						}
						else
						{
							$post_query_logic[] = "MATCH(post.titleindex, post.pagetextindex) AGAINST ('$wordlist' IN BOOLEAN MODE)";
						}
// vB 中文修改结束
			...
			else if ($vbulletin->options['fulltextsearch'] AND !$vbulletin->GPC['searchtype'])
			{
				...
				{
// vB 中文修改开始: 全文搜索
					$vbulletin->GPC['query'] = implode(' ', sp_str($vbulletin->GPC['query']));
					if ($vbulletin->GPC['titleonly'])
					{
						if ($vbulletin->GPC['sortby'] == 'rank')
						{
							$rank_select_logic = "MATCH(thread.titleindex) AGAINST ('" . $db->escape_string($vbulletin->GPC['query']) . "') AS score";
						}
						$thread_query_logic[] = "MATCH(thread.titleindex) AGAINST ('" . $db->escape_string($vbulletin->GPC['query']) . "')";
					}
					else
					{
						if ($vbulletin->GPC['sortby'] == 'rank')
						{
							$rank_select_logic = "MATCH(post.titleindex, post.pagetextindex) AGAINST ('" . $db->escape_string($vbulletin->GPC['query']) . "') AS score";
						}
						$post_query_logic[] = "MATCH(post.titleindex, post.pagetextindex) AGAINST ('" . $db->escape_string($vbulletin->GPC['query']) . "')";
					}
// vB 中文修改结束
...
// vB 中文修改开始：中文搜索 高亮修正
			// #############################################################################
			// get highlight words (part 3);
			if (!$vbulletin->options['searchposttable'] AND count($display['highlight']) % 2 == 0) 
			{
				if (!preg_match("/^[[:alpha:]]+$/", end($display['highlight']))) {
					$display['highlight']['temp'] = substr(end($display['highlight']), -3);
				}
			}
// vB 中文修改结束
...
// vB 中文修改开始：中文搜索 全文搜索
						...
						else
						{
							...
							if($oriquery) $replacedwords = preg_replace($patterns, $replacements, sanitize_search_query($oriquery, $errors));
							$display['words'] = explode("|||", str_replace("</u>OR<u>", "</u> OR <u>", $replacedwords));
						}

						if ($vbulletin->options['fulltextsearch'] OR $vbulletin->options['searchposttable'])
						{
