...
function print_cp_footer()
{
	...
	if ($vbulletin->debug)
	{
		...
		{
// vB 中文修改开始: 翻译
			echo "<p align=\"center\" class=\"smallfont\">SQL 查询 (" . $vbulletin->db->querycount . ") | " . (!empty($cvsversion) ? "$cvsversion | " : '') . "<a href=\"" . $vbulletin->scriptpath . iif(strpos($vbulletin->scriptpath, '?') > 0, '&amp;', '?') . "explain=1\">扩展</a></p>";
// vB 中文修改结束
		}
	...
	{
		echo '<p align="center">' . construct_phrase($vbphrase['vbulletin_copyright'], $vbulletin->options['templateversion'], date('Y')) . '</p>';
	}
...
// vB 中文: 将日期选择框放到“日”后
	echo "<script type=\"text/javascript\">new vB_DatePicker(\"{$name}_date\", \"{$name}_\", \"" . $vbulletin->userinfo['startofweek']  . "\"); </script>\r\n";
// vB 中文
}
...
function construct_help_button($option = '', $action = NULL, $script = '', $helptype = 0)
{
	...
	else
	{
		...
		{
			if ($vbulletin->debug AND preg_match('#example.com$#', $_SERVER['HTTP_HOST']))
			{
...
*/
function print_cp_message($text = '', $redirect = NULL, $delay = 1, $backurl = NULL)
{
	global $vbulletin, $vbphrase;
