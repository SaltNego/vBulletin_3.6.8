...
// vB 中文修改开始：中文搜索
		if ($wordlength < iif(function_exists('mb_strlen'),2,3) OR $wordlength > $vbulletin->options['maxsearchlength'])
		{
			return 0;
		}
// vB 中文修改结束
...
// vB 中文修改开始：中文搜索，索引词汇分数更新
		$vbulletin->db->query_write("UPDATE " . TABLE_PREFIX . "word SET score = score + 1 WHERE $getwordidsql");
// vB 中文修改结束
		...
		if ($newwords)
		{
// vB 中文修改开始：中文搜索，插入词汇更新时间
			$insertwords = "('" . str_replace(" ", "', ". TIMENOW ."),('", $newwords) . "', ". TIMENOW .")"; 
			/*insert query*/
			$vbulletin->db->query_write("INSERT IGNORE INTO " . TABLE_PREFIX . "word (title, dateline) VALUES $insertwords");
// vB 中文修改结束
