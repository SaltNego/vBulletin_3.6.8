...
function vbchop($string, $length)
{
	...
	{
		return msubstr($string, 0, $length);
	}
}
	...
	if ($vbulletin->options['enablecensor'] AND !empty($vbulletin->options['censorwords']))
	{
		...
		foreach ($censorwords AS $censorword)
		{
			if (substr($censorword, 0, 2) == '\\{')
			{
				...
				$text = preg_replace(
					'#(?<=[\x00-\x30\x5b-\x60\x7b-\x7f]|^)' . $censorword . '(?=[\x00-\x30\x5b-\x60\x7b-\x7f]|$)#si',
...
// vB 中文修改开始 标题截短避免出现乱码
		$titlenew = vbchop($title, $chars);
		// 计算标题中 ASCII 字符数目
		preg_match_all("/([\040-\176]+)/u", $titlenew, $matches);
		$enstr = '';
		for ($i=0; $i< count($matches[0]); $i++) {
			$enstr .= $matches[0][$i];
		}
		$enstrlen = strlen($enstr);

		if (vbstrlen($title) > $chars)
		{
			$chars += ceil($enstrlen/2);
			$titlenew = vbchop($title, $chars);
			if ($titlenew != $title)
			{
				$title = $titlenew;
...
// vB 中文修改开始: 显示北京时间
	...
	$tzoffset = iif(($tzoffset==8), $vbphrase['time_beijing'], $vbphrase['time_gmt'].$tzoffset);
// vB 中文修改结束
...
// #############################################################################
/**
* 中文二元语法(bigram)切分词 (UTF-8)
* @author	007pig
* @param	string	需要切分的字符串
*
* @return	array	切分好的词数组
*/
function sp_str($str)
{
	// 如果只是单字，则直接返回
	...
	foreach ($strarr as $str) 
	{
		if (preg_match("/^[[:alpha:]]+$/u", $str)) {
			...
			// 开始分中文词
			for ($i=0; $i<vbstrlen($str)-1; $i++)
			{
				array_push($ahz, msubstr($str, $i, 2));
			}
		}
	}
