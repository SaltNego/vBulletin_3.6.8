...
// vB ޸中文: 搜索
		$threads = $vbulletin->db->query_read_slave("
			SELECT thread.threadid, MATCH(thread.titleindex) AGAINST ('$safetitle') AS score
			FROM " . TABLE_PREFIX . "thread AS thread
			$hook_query_joins
			WHERE MATCH(thread.titleindex) AGAINST ('$safetitle')
				AND thread.open <> 10
...
// vB ޸中文: 避免一个中文字长度为 1，造成默认设置无法搜索
				if (strlen(preg_replace('#[\000-\031~!@\#$%^&*()=\[\]{}\\":;<>,.?/`]|\'{2,}#siu', '', $word)) < ($vbulletin->options['minsearchlength'] - 1))
// vB ޸中文
					{
						// word is too short
			...
			else
			{
				...
				# Can't get more specific without breaking other character sets
// vB ޸中文: 避免一个中文字长度为 1，造成默认设置无法搜索
				if (strlen(preg_replace('#[\000-\031~!@\#$%^&*()=\[\]{}\\":;<>,.?/`]|\'{2,}#si', '', $word)) < $vbulletin->options['minsearchlength'])
// vB ޸中文
				{
					// word is too short
					$word = htmlspecialchars_uni($word);
...
// vB ޸中文: 避免一个中文字长度为 1，造成默认设置无法搜索
			if (strlen(str_replace('*', '', $wordlower)) < ($vbulletin->options['minsearchlength'] - 1))
// vB ޸中文
			{
				// word is too short
				$word = htmlspecialchars_uni($word);
