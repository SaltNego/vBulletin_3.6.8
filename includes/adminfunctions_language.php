	...
	else
	{
		if ($languageid == 0)
		{
			...
			else
			{
				...
				vbflush();
				/*insert query*/
				// vB 中文修改开始，导入 imagesoverride 设置信息
				$vbulletin->db->query_write("
					INSERT INTO " . TABLE_PREFIX . "language (
						title, options, languagecode, charset,
						dateoverride, timeoverride, decimalsep, thousandsep,
						registereddateoverride, calformat1override, calformat2override, locale, logdateoverride, imagesoverride
					) VALUES (
						...
						'$langinfo[registereddateoverride]', '$langinfo[calformat1override]', '$langinfo[calformat2override]', '$langinfo[locale]', '$langinfo[logdateoverride]', '$langinfo[imagesoverride]'
					)
				");
				// vB 中文修改结束，导入 imagesoverride 设置信息
