...
define('CVS_REVISION', '$RCSfile$ - $Revision: 17556 $');
...
// vB ޸Ŀʼ: ȫ
	if ($vbulletin->options['fulltextsearch'])
	{
		if ($vbulletin->GPC['deleteindex'])
		{
			...
			{
				$db_alter->drop_field('titleindex');
				$db_alter->drop_field('pagetextindex');
			}
			else
			{
	...
	else
	{
		...
		if ($db_alter->fetch_table_info('post'))
		{
			...
			//  titleindex pagetextindex ֶ
			if(!$db_alter->add_index('title', array('titleindex', 'pagetextindex'), 'fulltext', true))
			{
				print_stop_message('dbalter_' . $db_alter->fetch_error(), $db_alter->fetch_error_message());
			}
		}
		...
		if ($db_alter->fetch_table_info('thread'))
		{
			...
			//  titleindex ֶ
			if (!$db_alter->add_index('title', array('titleindex'), 'fulltext', true))
			{
				...
				if ($db_alter->fetch_table_info('post'))
				{
					$db_alter->drop_index('titleindex');
				}
