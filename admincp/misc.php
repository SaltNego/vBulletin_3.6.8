...
define('CVS_REVISION', '$RCSfile$ - $Revision: 17441 $');
define('NOZIP', 1);
...
if ($_REQUEST['do'] == 'buildpostindex')
{
	...
	while ($post = $db->fetch_array($posts) AND (!$vbulletin->GPC['doprocess'] OR $vbulletin->GPC['totalposts'] < $vbulletin->GPC['doprocess']))
	{
		...
		{
			$pagetextindex = implode(' ', sp_str($post['pagetext']));
			$posttitleindex = implode(' ', sp_str($post['title']));
			$threadtitleindex = implode(' ', sp_str($post['threadtitle']));
			$db->query_write("UPDATE " . TABLE_PREFIX . "thread SET titleindex = '" . $db->escape_string($threadtitleindex) . "' WHERE threadid = $post[threadid]");
			$db->query_write("UPDATE " . TABLE_PREFIX . "post SET titleindex = '" . $db->escape_string($posttitleindex) . "', pagetextindex = '" . $db->escape_string($pagetextindex) . "' WHERE postid = $post[postid]");
		}
		else
		{
